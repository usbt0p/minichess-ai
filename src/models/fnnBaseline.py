...
class BaselineNet(nn.Module):
    ...
    def forward(self, x):
        ...
        value_result = self.value_result_head(
            x
        )  # logits without tanh if we predict 0/1/2
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        for features, moves, results, scores in train_loader:
            ...
            if model.result_mode == "classification":
                ...
                value_result_loss = value_result_criterion(
                    value_result.squeeze(-1), results.float()
                )
        ...
        val_res_acc = (
            correct_results / total_val_samples if total_val_samples > 0 else 0
        )
        ...
        print(
            f"  Train Loss: {total_loss:.4f} (Policy: {total_policy_loss:.4f}, Value: {total_value_loss:.4f})"
        )
        print(
            f"  Val Loss:   {val_loss:.4f} | Val Move Acc: {val_move_acc*100:.2f}% | Val Result Acc: {val_res_acc*100:.2f}%"
        )
    ...
    print(
        f"Best mean accuracy: {(best_move_acc + best_result_acc)/2*100:.2f}% achieved at epoch {best_epoch}"
    )
...
def plot_loss(train_losses, val_losses, val_move_accs, val_res_accs):
    ...
    plt.plot(
        ...
        [l[1] for l in train_losses],
        label="Train Policy Loss",
        linestyle="--",
    )
    plt.plot(
        range(len(train_losses)),
        [l[2] for l in train_losses],
...
if __name__ == "__main__":
    ...
    # load dataset
    dataset = MinichessFfnDataset(data_path, use_cache=True)
